...
def main():
    ...
    csv_path = os.path.join(project_root, 'code', 'stimuli_preprocessing',
                            'story_questions_mapping_pool.csv')
    ...
    while True:
        start_input = input(
            f"\nEnter story number to start from "
            f"(1-{len(stories_df)}, or press Enter for story 1): ").strip()
    ...
    # %% Run experiment
    n_bits_story = int(np.ceil(np.log2(8)))  # 8 stories
    ...
    with ExperimentController(**ec_args) as ec:
        ...
        for story_idx, story_row in stories_df.iterrows():
            ...
            if story_audio[story_id] is not None:
                ...
                # Show fixation cross for 1 second before story starts
                ...
                # Redraw cross after starting stimulus
                ec.screen_text("+", pos=(0.75, 0), units='norm',
                               color='white', font_size=64)
                ec.flip()
                ec.wait_secs(0.1)
            ...
            for q_idx, q_data in enumerate(questions):
                ...
                if q_data['audio'] is not None:
                    ...
                    # Display question and options after starting audio
                    ec.screen_text(
                        f"Question {q_data['question_num']} of 5",
                        pos=[0, 0.8], units='norm', color='w', font_size=18)

                    if options is None:
                        ec.screen_text(
                            q_data['question_text'], pos=[0, 0.4],
                            units='norm', color='w', font_size=32, wrap=True)
                    else:
                        ec.screen_text(
                            q_data['question_text'], pos=[0, 0.5],
                            units='norm', color='w', font_size=28, wrap=True)
                        y_start = 0.0
                        y_spacing = 0.15
                        for i, option_text in enumerate(options):
                            y_pos = y_start - (i * y_spacing)
                            ec.screen_text(
                                option_text, pos=[0, y_pos], units='norm',
                    ...
                    # Trigger
                    ec.stamp_triggers([(b + 1) * 4 for b in
                                       decimals_to_binary(
                                           [story_idx, q_idx],
                                           [n_bits_story, n_bits_question])])
                    ...
                    # Redraw question so it stays visible after audio ends
                    ec.screen_text(
                        f"Question {q_data['question_num']} of 5",
                        pos=[0, 0.8], units='norm', color='w', font_size=18)
